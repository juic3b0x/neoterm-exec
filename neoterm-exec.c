...
#ifndef NEOTERM_BASE_DIR
# define NEOTERM_BASE_DIR "/data/data/io.neoterm/files"
#endif
...
static const char* neoterm_rewrite_executable(const char* filename, char* buffer, int buffer_len)
{
	if (starts_with(filename, NEOTERM_BASE_DIR) ||
			starts_with(filename, "/system/"))
		return filename;

	strcpy(buffer, NEOTERM_PREFIX "/bin/");
	...
	if (bin_match == filename || bin_match == (filename + 4)) {
		...
		strncpy(buffer + sizeof(NEOTERM_PREFIX "/bin/") - 1, bin_match + 5, buffer_len - sizeof(NEOTERM_PREFIX "/bin/"));
		filename = buffer;
	}
	return filename;
}
...
{
	bool android_10_debug = getenv("NEOTERM_ANDROID10_DEBUG") != NULL;
	if (android_10_debug) {
		printf("execve(%s):\n", filename);
	...
	filename = neoterm_rewrite_executable(filename, filename_buffer, sizeof(filename_buffer));
	...
	*newline_location = 0;
	...
	char interp_buf[512];
	const char* new_interpreter = neoterm_rewrite_executable(interpreter, interp_buf, sizeof(interp_buf));
...
final:
	...
	bool android_10_wrapping = getenv("NEOTERM_ANDROID10") != NULL;
	if (android_10_wrapping) {
		char realpath_buffer[PATH_MAX];
		bool realpath_ok = realpath(filename, realpath_buffer) != NULL;
		if (realpath_ok) {
			bool wrap_in_proot = (strstr(realpath_buffer, NEOTERM_BASE_DIR) != NULL);
			if (android_10_debug) {
				printf("neoterm-exec: realpath(\"%s\") = \"%s\", wrapping=%s\n", filename, realpath_buffer, wrap_in_proot ? "yes" : "no");
			}
			if (wrap_in_proot) {
				orig_argv_count = 0;
				while (argv[orig_argv_count] != NULL) orig_argv_count++;
				...
				filename = NEOTERM_PREFIX "/bin/proot";
				new_argv[0] = "proot";
